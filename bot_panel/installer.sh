...
surat=$(curl -sS https://raw.githubusercontent.com/kenzotelco/ip-bot/main/bot | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
dircreate
        ...
        echo -e "SAMVPN Bot Panel Installer
        "
        [[ ! -f /root/ResBotAuth ]] && {
        ...
        echo -ne "Username admin panel use '@' [Ex: @sam_sfx] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@sam_sfx"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [dafult: SAMVPN-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="SAMVPN-STORE"
        echo ""
...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -qO- bot.offizial-kenzo.me/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
			}
